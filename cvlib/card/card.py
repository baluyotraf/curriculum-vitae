...
class Card:
    ...
    def _optimize_content_size(
        self, content: TextLike, area: Tuple[int, int]
    ) -> TextLike:
        ...
        if content_ratio >= 1.0:
            ...
            size = bound_optimization(
                content_ratio, 1.0, func, limit=area[1], tolerance=0.5
            )
    ...
    def render(self, content: TextLike) -> Image.Image:
        ...
        content.render(draw, (self.margin, self.margin + offset), fill=self.foreground)
        return img
